...
# ===== SQLite 持久化（适配 Railway /data Volume）=====
...
def get_user(user_id: int):
    ...
    row = cur.fetchone()
    if row:
        return {"score": row[0], "correct": row[1], "wrong": row[2]}
    else:
...
async def stop(interaction: discord.Interaction):
    ...
    user_id = interaction.user.id
    user = get_user(user_id)   # ← 每次都从数据库读取最新分数
    async with await get_user_lock(user_id):
...
async def on_ready():
    init_db()
    await tree.sync()
    print("✅ 机器人已启动 | 多用户持久化已启用 | 数据库位置：/data/kana_data.db")
